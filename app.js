// ZetaChain Universal Portfolio Tracker - AI-Powered Frontend
// Volatility Premium Algorithm & Architecture Generated by Amazon Q
...
class PortfolioTracker {
    ...
    setupEventListeners() {
        ...
        // Emergency withdraw listener
        document.getElementById('emergencyWithdrawBtn').addEventListener('click', () => this.emergencyWithdraw());
        document.getElementById('emergencyPositionSelect').addEventListener('change', (e) => {
            document.getElementById('emergencyWithdrawBtn').disabled = !e.target.value;
        });
    }
    ...
    displayPositions(positions) {
        ...
        const statusNames = ['Active', 'Withdrawn', 'Refunded', 'Failed'];
        const statusClasses = ['status-active', 'status-withdrawn', 'status-refunded', 'status-failed'];
    ...
    updatePositionSelect(positions) {
        ...
        const emergencySelect = document.getElementById('emergencyPositionSelect');
        
        select.innerHTML = '<option value="">Select a position</option>';
        emergencySelect.innerHTML = '<option value="">Select a position</option>';
        ...
        positions.forEach((position, index) => {
            if (position.status === 0) {
                ...
                // Also add to emergency select
                const emergencyOption = option.cloneNode(true);
                emergencySelect.appendChild(emergencyOption);
            }
        });
        
        const hasActivePositions = positions.filter(p => p.status === 0).length > 0;
        select.disabled = !hasActivePositions;
        emergencySelect.disabled = !hasActivePositions;
    ...
    async emergencyWithdraw() {
        try {
            const positionId = document.getElementById('emergencyPositionSelect').value;
            
            if (!positionId) {
                this.showError('Please select a position for emergency withdrawal');
                return;
            }
            
            const confirmed = confirm(
                'WARNING: Emergency withdrawal will mark this position as Failed and immediately return funds. ' +
                'Only use this if normal withdrawal is not working. Continue?'
            );
            
            if (!confirmed) return;
            
            this.showLoading('Processing emergency withdrawal...');
            
            const tx = await this.contract.emergencyWithdraw(positionId);
            await tx.wait();
            
            this.showSuccess('Emergency withdrawal successful! Funds returned to your wallet.');
            this.addTransactionStatus(tx.hash, 'Emergency Withdrawal');
            
            document.getElementById('emergencyPositionSelect').value = '';
